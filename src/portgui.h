...
#define UI_WINDOW_FULLSCREEN (1 << 4)
	UIElement e;
...
bool UIDrawPixel(UIPainter *painter, int x, int y, uint32_t color);
...
UIWindow *UIWindowCreate(UIWindow *owner, uint32_t flags, const char *cTitle, int _width, int _height) {
	...
	if (flags & UI_WINDOW_FULLSCREEN) {
    	Atom wmState = XInternAtom(ui.display, "_NET_WM_STATE", False);
    	Atom fullscreen = XInternAtom(ui.display, "_NET_WM_STATE_FULLSCREEN", False);

    	XEvent e = {0};
    	e.xclient.type = ClientMessage;
    	e.xclient.window = window->os->window;
